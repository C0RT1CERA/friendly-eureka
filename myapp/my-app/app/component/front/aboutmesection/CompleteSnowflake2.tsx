...
// Snowflake Component
export function CompleteSnowflake2() {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-gray-900">
      ...
      >
        {/* Smaller Snowflakes (Static) */}
        {positions.map((pos, i) => (
          <motion.g
            key={i}
            transform={`translate(${pos.x - smallFlakeSize / 2}, ${pos.y - smallFlakeSize / 2}) scale(0.33)`}
          >
            <path d={generateSnowflake()} fill="none" stroke="#F87171" strokeWidth="2" />
          </motion.g>
        ))}

        {/* Main Snowflake (Rotating) */}
        <motion.g
          animate={{ rotate: 360 }}
          transition={{ repeat: Infinity, duration: 8, ease: "linear" }}
        >
          <path d={generateSnowflake()} fill="none" stroke="#60A5FA" strokeWidth="1" />
        </motion.g>
      </motion.svg>
    </div>
  );
